...
default persistent.use_regular_expressions = False
    ...
    class Search(python_object):
        ...
        def IsMatch(self, src, target):
            ...
            if persistent.use_regular_expressions:
                ...
                if persistent.search_exact_match:
                    ...
                    found = all(t in msg for t in target)
            return found
        ...
        def FindText(self, direction=1):
            ...
            if self.found_idxs is None:
                ...
                if not persistent.search_exact_match and not persistent.use_regular_expressions:
                    target = target.split(" ")
        ...
        def CreateDisplayableEntry(self, idx, highlight=False):
            ...
            found = False
            if self.found_idxs and (idx in self.found_idxs) and self.found_idxs[idx]:
                found = True
                ...
                if self.search_text == "" or self.search_text == " " or self.search_text is None:
                    ...
                    if persistent.use_regular_expressions:
                        ...
                        if persistent.search_exact_match:
                            matches = [(source.find(target), source.find(target) + len(target))]
                        else:
                            matches = []
                            target = target.split(" ")
                            for t in target:
                                start = source.find(t)
                                matches.append((start, start + len(t)))
                            matches.sort()
                    ...
                    if matches:
                        ...
                        for start, end in matches:
                            what += msg[prev_end:start]
                            what += "{u}{color=#f88}{outlinecolor=#000}" + msg[start:end] + "{/outlinecolor}{/color}{/u}"
                            prev_end = end
                        what += msg[prev_end:]
                    else:
                        what = msg
            ...
            frame:
                ...
                vbox:
                    ...
                    vbox:
                        ...
                        textbutton _("Regular Expressions") action ToggleField(persistent, "use_regular_expressions"), Function(search.InputChanged) tooltip "Use regular expressions." activate_sound "sounds/sys10.wav"
                        button:
